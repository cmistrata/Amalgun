...
public class Cannon2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (AutoFiring && GameManager.Instance.State == GameState.Fighting && _teamTracker.Team != Team.Neutral) {
    ...
    Vector2 GetAimingDirection() {
        switch (_currentTargetingStrategy) {
            case TargetingStrategy.TargetPlayer:
                return Player2D.Instance != null ? Player2D.Instance.transform.position - transform.position : _aimingDirection;
